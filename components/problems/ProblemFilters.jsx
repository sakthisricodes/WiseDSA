...
export function ProblemFilters({ companies = [], techniques = [] }) {
  ...
  return (
    <div className="space-y-3.5 rounded-xl border border-zinc-800/80 dark:border-zinc-800 bg-zinc-900/50 p-4 backdrop-blur-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-9 h-9 bg-zinc-950/60 border-zinc-800 text-xs"
          />
          {search && (
            <button
        ...
        {hasActiveFilters && (
          <Button
            ...
            className="text-xs text-zinc-400 hover:text-white shrink-0 gap-1.5 h-9 px-2.5"
          >
            <X className="h-3.5 w-3.5" />
            Clear
          </Button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2.5 pt-1">
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400 flex items-center gap-1 h-3.5 truncate">
            <Layers className="h-3 w-3 shrink-0" /> Topic
          </label>
          <select
            value={topic}
            onChange={(e) => updateParam('topic', e.target.value)}
            className="w-full h-9 rounded-lg border border-zinc-800 bg-zinc-950 px-2.5 py-1 text-xs text-zinc-200 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors cursor-pointer"
          >
